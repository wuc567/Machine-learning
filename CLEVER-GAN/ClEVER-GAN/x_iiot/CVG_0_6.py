...
for epoch in range(epochs):
    ...
    # loss_cos_vae = alpha * loss_generator_vae + loss_generator_gan + alpha * cos_loss
...
# # 转换为numpy数组并保存为csv文件
# generated_data_numpy = generated_data.cpu().numpy()
# np.savetxt('4_713_best_vae_gan_contra_lossMethod_generated_data.csv', generated_data_numpy, delimiter=',')
...
plt.figure(figsize=(10, 5))
plt.plot(coscos, label='cos Loss')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Cos Loss')
plt.legend()
...
# df.to_csv('D:/python/pythonProject/PHDfirstTest/x_iiot/List_txt/Xiiot_0_7_similarity_lists.txt', sep='\t',
#           index=False)
