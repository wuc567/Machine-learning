...
for epoch in range(epochs):
    ...
    real_data_split = real_data

    dis1 = discriminator(real_data_split)
    dis2 = discriminator(gene_data)

    # print("dis2",dis2)
    # print("dis1.shape", dis1.shape)
    dis1 = dis1.reshape(batch_size, -1)
    dis2 = dis2.reshape(batch_size, -1)

    loss_discri_real = gan_loss(dis1, real_label)
    loss_discri_fake = gan_loss(dis2, fake_label)

    loss_discri_total = loss_discri_real + loss_discri_fake

    optimizer_d.zero_grad()
    loss_discri_total.backward(retain_graph=True)
    optimizer_d.step()
    ...
    # gene_data1 = gene_data.reshape(batch_size, -1)
    # loss_gene = gene_loss(gene_data1, real_data_split)
    loss_gene = gan_loss(discriminator(gene_data).reshape(batch_size, -1), real_label)
    optimizer_g.zero_grad()
    loss_gene.backward()
    optimizer_g.step()

    # 打印训练过程中的损失
    print(f"Epoch [{epoch + 1}/{epochs}], discriminator Loss: {loss_discri_total.item():.4f}")
    print(f"Epoch [{epoch + 1}/{epochs}], generator Loss: {loss_gene.item():.4f}")
    # -------------------------------------------------------------------------------------------------------------------------------------------
    # -------------------------------------------------------------------------------------------------------------------------------------------
    # -------------------------------------------------------------------------------------------------------------------------------------------
    # -------------------------------------------------------------------------------------------------------------------------------------------
...
np.savetxt('D:/python/pythonProject/PHDfirstTest/x_iiot/GAN/9_5960_acgan_generated_data.csv', best_generated_data_numpy,
           delimiter=',')
