...
from .cascade.cascade_classifier_thresh import   CascadeClassifier_th
from .config import GCTrainConfig
from .fgnet import FGNet
from .utils.log_utils import get_logger

LOGGER = get_logger("gcforest.gcforest_cs")


class DBCForest(object):
    def __init__(self, config):
        ...
        if "net" in self.config:
            ...
            self.fg = None
        if "cascadeCS" in self.config:
            self.ca = CascadeClassifier_th(self.config["cascadeCS"])
        else:
            self.ca = None
    ...
    def fit_transform(self, X_train, y_train, X_test=None, y_test=None, train_config=None):
        ...
        if self.ca is not None:
            a = self.ca.fit_transform(X_train, y_train, X_test, y_test, train_config=train_config)
    ...
    def predict_proba(self, X, y=None):
        if self.fg is not None:
            X = self.fg.transform(X)
        y_proba = self.ca.predict_proba(X, y)
        return y_proba
