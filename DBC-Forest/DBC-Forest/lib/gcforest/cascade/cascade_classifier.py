...
def calc_accuracy(y_true, y_pred, name, prefix=""):###计算准确率
...
def get_opt_layer_id(acc_list):
    ...
    opt_layer_id = np.argsort(-np.asarray(acc_list), kind='mergesort')[0]###argsort()是将X中的元素从小到大排序后，提取对应的索引index，然后输出到y
    return opt_layer_id
...
class CascadeClassifier(object):
    def __init__(self, ca_config):
        ...
        ----------
        early_stopping_rounds: int  停止增长标记
        ...
        data_save_dir: str [default=None]
            each data_save_rounds save the intermidiate results in data_save_dir  每个data_save_round将中间结果保存在data_save_dir中
            if data_save_rounds = 0, then no savings for intermidiate results  如果data_save_rounds = 0，则中间结果没有任何保存
        """
        ...
        try:
            ...
            while 1:
                ...
                if layer_id == 0:
                    ...
                    X_cur_train = X_proba_train.copy()###上一层的预测值
                ...
                for _i, i in enumerate(look_indexs):
                    X_cur_train = np.hstack((X_cur_train, X_train[:, group_starts[i]:group_ends[i]]))###填充数据
