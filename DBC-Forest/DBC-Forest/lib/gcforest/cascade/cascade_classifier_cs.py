...
class CascadeClassifier_cs(object):
    ...
    def _check_group_dims(self, X_groups, is_fit):
        ...
        for i, X_group in enumerate(X_groups):
            ...
            if is_fit:
                ...
                print("train --- shape{}".format(X_group.shape))
                print("train_group --- {}".format(X_group.shape[1]))
                group_starts.append(0 if i == 0 else group_ends[i - 1])
                group_ends.append(group_starts[i] + group_dims[i])
            else:
                #print("X_group shape[1] {} ----- group_dims shape[i] {} ".format(X_group.shape[1],group_dims[i]))
    ...
    def confidence_screening(self, y_train_proba_li, y_train, train_index_now, layer_id):
        CX_thresh, CX_train = decide_thresh(y_train_proba_li=y_train_proba_li, y_train=y_train,
                                            part_decay=self.part_decay)
    ...
    def transform(self, X_groups_test,y_test=None):
        print("X_groups length of X_groups is {}".format(len(X_groups_test)))
