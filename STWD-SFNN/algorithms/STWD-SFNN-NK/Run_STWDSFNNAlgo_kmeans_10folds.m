function [Contrast_SFNN_TraiNums,Contrast_Result_matrix, Contrast_Result_Mean_all,Contrast_Result_SE_all] = Run_STWDSFNNAlgo_kmeans_10folds(data, label, Para_Init, indices)
...
for k = 1:10 %Para_Init.Data_epochs 
    ...
    [Test_Result, Train_num_turns] = arrayfun(@(p1,p2,p3) STWDSFNNAlgorithm_kmeans_10folds(Train,Validate,Test,Para_Init,p1,p2,p3), Para_Optimize.alpha,Para_Optimize.BatchSize,Para_Optimize.lambda,'UniformOutput',false);  % 1*48  
...
% 保存实验结果
save([file_path_save  char(file_name_per)  '_STWDSFNN_nokmeans_'  num2str(Para_Init.s) num2str(Para_Init.t)  '.mat'], ...
