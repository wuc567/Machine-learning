...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 加载预存的 stwd %%%%%%%%%%%%%%%%%%%%%%%%%%%%
load([file_path_read  char(file_name_per)  '_STWDSFNN_para_stwd'  '.mat'])
Para_Init.STWD_lambda_cell = STWD_lambda_cell(100:110);
...
for i = 1:length(active_list)
    ...
    for j = 1:length(data_type)
        ...
        [Result_TraiNums, Result_matrix, Result_Mean,Result_SE] = Run_STWDSFNNAlgo_kmeans_10folds(data, label, Para_Init, indices);
...
% 保存实验结果
save([file_path_save  char(file_name_per)  '_STWDNK_10folds_all'  '.mat'], 'Result_matrix_all','Result_train_all')
