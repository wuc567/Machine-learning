function [Test_Result, Train_num_turns] = STWDSFNNAlgorithm_kmeans_10folds(Train,Validate,Test,Para_Init,alpha,BatchSize,lambda)
...
%     Para_Init.train_num_turns(Para_Init.Train_num,:) = [aa, ab, ac];   % 开始处理的样本数, sfnn处理的样本数, stwd处理的样本数
    ...
    % 序贯三支的结果损失
    Cost_Result = [Cost_Result, TWD_Result.Cost];  % cell2mat(）
end
...
% 保存实验结果
save([file_path_save  char(file_name_per)  '_STWDSFNN_Para_nokeans_'  num2str(Para_Init.s) num2str(Para_Init.t)  '.mat'], ...
...
[~,~,Disc_X_index] = unique(TWD.TrainX,'rows');  
TWD.Disc_X_Equc = splitapply(@(x){x}, find(Disc_X_index), Disc_X_index); % n*1
