function [Contrast_SFNN_TraiNums, Contrast_Result_matrix, Contrast_Result_Mean_all,Contrast_Result_SE_all] = Run_TWDSFNNAlgo_10folds(data, label, Para_Init, indices)
...
for k=1:10 %Para_Init.Data_epochs 
    ...
    [Test_Result, Train_num_turns] = arrayfun(@(p1,p2,p3) TWDSFNNAlgorithm_10folds(Train,Validate,Test,Para_Init,p1,p2,p3), Para_Optimize.alpha,Para_Optimize.BatchSize,Para_Optimize.lambda,'UniformOutput',false);  % 1*48  
...
% 保存实验结果
save([file_path_save  char(file_name_per)  '_TWDSFNN_'  num2str(Para_Init.s) num2str(Para_Init.t)  '.mat'], ...
    'Contrast_SFNN_Result','SFNN_Result', 'Contrast_Result_matrix',...
     'Contrast_Result_Mean_all', 'Contrast_Result_SE_all', 'Para_Init', 'Contrast_SFNN_TraiNums')
...
% 保存实验过程变量
save([Para_Init.file_path_save  char(Para_Init.file_name_per)  '_TWDSFNN_Para_Process_'  num2str(Para_Init.s) num2str(Para_Init.t)  '.mat'], 'Acc_Result','Acc_value')
