function [Test_Result, Train_num_turns] = TWDSFNNAlgorithm_10folds(Train,Validate,Test,Para_Init,alpha,BatchSize,lambda)
...
while ~isempty(Para_Init.TWD_Next_Train)  % 0代表 BND 集合中仍有元素
    ...
    Para_Init.TWD_InputNum = length(Para_Init.TWD_Next_Train);     
...
toc
Test_time = toc;
Test_Result = [Test_Acc,Test_WeightF1,Test_Kappa,Test_Loss,Para_Init.Train_num,Train_time,Test_time];
% fprintf('三支决策神经网络的测试集的准确率=%.4f\n',Test_Acc)
save([Para_Init.file_path_save char(Para_Init.file_name_per)  '_TWDSFNN_Para_'  num2str(Para_Init.s) num2str(Para_Init.t)],...
